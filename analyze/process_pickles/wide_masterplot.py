...
def master_plot(file,mode='lines',threshold=False,
  ...
  if ts.pmc_type == 'intel_snb' :
    # Plot key 1
    plot(ax[1],ts,[8,9],3600.,1e9,
         ylabel='Total AVX +\nSSE Ginst/s')
    
    # Plot key 2
    plot(ax[3],ts,[0,1],3600.,1.0/64.0*1024.*1024.*1024.,
         ylabel='Total Mem BW GB/s')

    #Plot key 3
    #plot(ax[2],ts,[2],3600.,1.0/64.0*1e9, ylabel='L1 BW GB/s')
    plot(ax[5],ts,[10,-11],3600.,1024.0*1024.0*1024.0, ylabel='Memory Usage GB',
         do_rate=False)
  ...
  # Plot lnet sum rate
  plot(ax[7],ts,[3,4],3600.,1024.**2,ylabel='Total lnet MB/s')

  # Plot remaining IB sum rate
  plot(ax[9],ts,[5,6,-3,-4],3600.,1024.**2,ylabel='Total (ib_sw-lnet) MB/s') 

  #Plot CPU user time
  plot(ax[11],ts,[7],3600.,wayness*100.,
       xlabel='Time (hr)',
