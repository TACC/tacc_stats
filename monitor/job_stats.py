...
class Host(object):
    ...
    def __init__(self, job, name, raw_stats_dir):
        self.job = job
        self.name = name
        self.times = []
        self.marks = {}
    ...
    def get_stats_paths(self):
        raw_host_stats_dir = os.path.join(self.raw_stats_dir, self.name)
...
class Job(object):
    ...
    __slots__ = ('id', 'start_time', 'end_time', 'acct', 'schemas', 'hosts', 'times','stats_home')

    def __init__(self, acct, stats_home):
        self.id = acct['id']
    ...
    def gather_stats(self):
        path = get_host_list_path(self.acct, self.stats_home + '/hostfiles')
        if not path:
        ...
        for host_name in host_list:
            # TODO Keep bad_hosts.
            host = Host(self, host_name, self.stats_home + '/archive' )
...
def from_acct(acct, stats_home):
    """from_acct(acct, stats_home)
    Return a Job object constructed from the SGE accounting data acct using
    stats_home as the base directory, running all required processing.
    """
    job = Job(acct, stats_home)
