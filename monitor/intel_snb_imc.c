...
#define MC_CTL2           0xE0
#define MC_CTL3           0xE4
//#define MC_FIXED_CTR     0xD4
#define MC_FIXED_CTR     0xD0
#define MC_CTR0         0xA0
//#define MC_CTR0         0xA4
#define MC_CTR1         0xA8
//#define MC_CTR1         0xAC
...
static int intel_snb_imc_begin_dev(char *bus_dev, uint32_t *events, size_t nr_events)
{
  ...
  ctl = 0x40000UL; // remove reset flag from fixed counter
  if (pwrite(pci_fd, &ctl, sizeof(ctl), MC_FIXED_CTL) < 0) {
    ERROR("cannot enable freeze of C-box counter: %m\n");
    goto out;
  }
  ...
  /* Manually reset programmable MC counters. They are 4 apart, but each counter register 
     is split into 2 32-bit registers, A and B */
  int zero = 0x0UL;
  for (i = 0; i < 2*nr_events; i++) {
    if (pwrite(pci_fd, &zero, sizeof(zero), MC_CTR0 + 4*i) < 0) { 
      ERROR("cannot reset counter %08X through `%s': %m\n", 
	    (unsigned) MC_CTL0 + 4*i,
            pci_path);
      goto out;
    }
...
#define X(k,r...) \
  ({ \
    ...
    uint64_t val = 0x0ULL; \
    if ( pread(pci_fd, &val_a, sizeof(val_a), MC_##k+4) < 0 || pread(pci_fd, &val_b, sizeof(val_b), MC_##k) < 0 ) \
      ERROR("cannot read `%s' (%08X,%08X) through `%s': %m\n", #k, MC_##k+4, MC_##k, pci_path); \
    else \
