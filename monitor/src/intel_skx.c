...
static int intel_skx_begin(struct stats_type *type)
{
  int n_pmcs = 0;
  int nr = 0;
...
static void intel_skx_collect(struct stats_type *type)
{
  int i;
...
struct stats_type intel_skx_stats_type = {
  .st_name = "intel_skx",
  .st_begin = &intel_skx_begin,
  .st_collect = &intel_skx_collect,
#define X SCHEMA_DEF
