...
/*! \brief Event select 
  ...
  -# Define event below
  -# Modify events array in intel_hsw_ht_begin()
*/
...
static int intel_hsw_ht_begin_cpu(char *cpu, uint64_t *events, size_t nr_events)
{
  int rc = -1;
  char msr_path[80];
  int msr_fd = -1;
...
static int intel_hsw_ht_begin(struct stats_type *type)
{
  int nr = 0;
  ...
  for (i = 0; i < nr_cpus; i++) {
    ...
    if (cpu_is_haswell(cpu))
      if (intel_hsw_ht_begin_cpu(cpu, events, 4) == 0)
	nr++;
  }
...
static void intel_hsw_ht_collect_cpu(struct stats_type *type, char *cpu)
{
...
static void intel_hsw_ht_collect(struct stats_type *type)
{
  int i;
  for (i = 0; i < nr_cpus; i++) {
    char cpu[80];
    ...
    if (cpu_is_haswell(cpu))
      intel_hsw_ht_collect_cpu(type, cpu);
  }
}

//! Definition of stats entry for this type
struct stats_type intel_hsw_ht_stats_type = {
  .st_name = "intel_hsw_ht",
  .st_begin = &intel_hsw_ht_begin,
  .st_collect = &intel_hsw_ht_collect,
#define X SCHEMA_DEF
