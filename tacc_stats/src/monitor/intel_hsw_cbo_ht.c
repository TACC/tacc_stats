...
/*! \brief Event select
  ...
  -# Define event below
  -# Modify events array in intel_hsw_cbo_ht_begin()
*/
...
static int intel_hsw_cbo_ht_begin_box(char *cpu, int box, uint64_t *events, size_t nr_events)
{
  int rc = -1;
  char msr_path[80];
  int msr_fd = -1;
...
static int intel_hsw_cbo_ht_begin(struct stats_type *type)
{
  int nr = 0;
  ...
  for (i = 0; i < nr_cpus; i++) {
	...
	for (box = 0; box < nr_cpus/2; box++)
	  if (intel_hsw_cbo_ht_begin_box(cpu, box, cbo_events[box], 4) == 0)
	    nr++;
      }
  }
...
static void intel_hsw_cbo_ht_collect_box(struct stats_type *type, char *cpu, char* cpu_box, int box)
{
...
static void intel_hsw_cbo_ht_collect(struct stats_type *type)
{
  ...
  for (i = 0; i < nr_cpus; i++) {
    ...
    if (cpu_is_haswell(cpu))
      {
	for (box = 0; box < nr_cpus/4; box++)
	  {
	    snprintf(cpu_box, sizeof(cpu_box), "%d/%d", socket_id, box);
	    intel_hsw_cbo_ht_collect_box(type, cpu, cpu_box, box);
	  }
      }
  }
}
...
struct stats_type intel_hsw_cbo_ht_stats_type = {
  .st_name = "intel_hsw_cbo_ht",
  .st_begin = &intel_hsw_cbo_ht_begin,
  .st_collect = &intel_hsw_cbo_ht_collect,
#define X SCHEMA_DEF
