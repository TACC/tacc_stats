...
static PyObject*
_main(PyObject *self, PyObject *args)
{
  int lock_fd = -1;
  int lock_timeout = 30;
  ...
  int argc = PyTuple_Size(args)+1;
  
  char *argv[argc];

  if (!PyArg_ParseTuple(args, "s|s", &argv[1], &argv[2]))
    return NULL;
...
static PyMethodDef MonitorMethods[] = {
  {"monitor", (PyCFunction) _main, METH_VARARGS, "Collect resource usage data."},
  {NULL, NULL, 0, NULL}
};
...
initmonitor(void)
{
  (void) Py_InitModule("monitor", MonitorMethods);
}
