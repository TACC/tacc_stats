...
### Test plotters
filelist = [os.path.join(os.path.dirname(os.path.abspath(__file__)),'20_ref')]
print(filelist)
...
def dev_test():
    dev_plot = plots.DevPlot({'intel_nhm' : ['mem']}, {'intel_nhm' : ['MemUsed']},
                             outdir='.',
                             prefix='MemUsed',
                             header='Plot all events for a device',lariat_data='pass',
                             save=True)
...
#### Heat Map plotter
def heat_test():
    heat_plot = plots.HeatMap(k1=['intel_nhm','intel_nhm'], 
